...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    $u   = mysqli_fetch_assoc(mysqli_query($koneksi,"SELECT * FROM users WHERE id='$uid'"));
    if (!password_verify($old,$u['password'])) {
        $msg="Password lama salah."; $msg_type="error";
    } elseif (strlen($new)<6) {
        ...
        mysqli_query($koneksi,"UPDATE users SET password='$hash' WHERE id='$uid'");
...
<div class="main-wrap">
    <div class="topbar">
        <div class="topbar-title"><i class="fas fa-key" style="color:var(--secondary);margin-right:8px"></i>Ganti Password</div>
    ...
    <div class="content">
        <div class="form-card" style="max-width:480px">
            <h3><i class="fas fa-lock" style="color:var(--secondary);margin-right:8px"></i>Ubah Password Admin</h3>
            <?php if($msg): ?><div class="alert alert-<?=$msg_type?>"><i class="fas fa-info-circle"></i> <?=$msg?></div><?php endif; ?>
            <form method="POST">
                <div class="fg"><label>Password Lama</label><input type="password" name="old_password" required placeholder="Password saat ini"></div>
                <div class="fg"><label>Password Baru</label><input type="password" name="new_password" required placeholder="Minimal 6 karakter"></div>
                <div class="fg"><label>Konfirmasi Password Baru</label><input type="password" name="confirm_password" required placeholder="Ulangi password baru"></div>
